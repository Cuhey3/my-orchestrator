    ...
    if (continueElementName != null) {
      while (true) {
        ...
        if (continueElements.isEmpty()) {
          ...
          get = Jsoup.connect(requestUrl + "&" + continueElementName + "=" + continueElementValue).timeout(Integer.MAX_VALUE).get();
          addElementsAsMap(resultMapList, get.select(listName).select(mapName));
        }
      }
    }
    ...
    elements.stream()
            .map((element) -> {
              ...
              StreamSupport.stream(element.attributes().spliterator(), false)
                      .filter((entry) -> ignoreFieldNameList.isEmpty() || !ignoreFieldNameList.contains(entry.getKey()))
                      .forEach((entry) -> {
