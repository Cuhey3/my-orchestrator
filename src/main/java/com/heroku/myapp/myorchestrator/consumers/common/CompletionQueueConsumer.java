...
public class CompletionQueueConsumer extends QueueConsumer {
    ...
    public void configure() {
        ...
        for(Kind k : Kind.values()){
            if(k.optionIsEnable() && k.isEnable(KindOptions.affect)){
                choice.when(messageKindIs(k))
                        .multicast().to(k.affectQueueUriArray()).endChoice();
            }
        }
        /*choice.filter(util().loadAffectPredicate())
                .split().body()
